...
export const Tareas = () => {
	...
	const handleDelete = (index) => {
		...
		const realIndex = store.tasks.findIndex(t => t === task);
		if (realIndex !== -1) {
			dispatch({ type: "delete_task", payload: realIndex });
		}
	};
	...
	const filteredTasks = tasks.filter((t) => {
		...
		if (hogar && t.hogarId !== hogar.id) return false;
		return true;
	});
	return (
		<>
							...
							{filteredTasks.length === 0 ? (
								...
								filteredTasks.map((t, i) => (
									<div key={i} className="col-12 col-md-6 col-lg-4 mb-4">
										<div className={`card p-3 h-100 text-start bg-ivory ${t.hecha ? "hecha" : "pendiente"}`}>
											<h5 className={t.hecha ? "text-decoration-line-through" : ""}>{t.nombre}</h5>
											<div className="d-flex flex-wrap gap-2 mb-2">
												<p className="mb-0">
													<strong>Asignada a:</strong>{" "}
													{t.asignadaA === "equipo" ? t.miembros?.join(", ") || "Sin miembros" : "Usuario"}
												</p>
											...
											<div className="mt-auto text-end">
												<button
													...
													onClick={() => {
														const realIndex = store.tasks.findIndex(task => task === t);
														if (realIndex !== -1) {
															dispatch({ type: "toggle_task_done", payload: realIndex });
														}
													}}
												>
						...
						<div className="modal-content bg-coral rounded border-charcoal p-3">
							<div className="modal-header border-0">
								<h2 className="modal-title charcoal w-100 text-center m-0">
									{editIndex !== null ? store.tasks[editIndex]?.nombre || "Editar Tarea" : "Nueva Tarea"}
								</h2>
